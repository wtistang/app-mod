...
2. From the browser window, type the **GitLab** as **10.0.0.1:82** and press **Enter**.

  ![](images/argocd-gitlab-url.png) 
3. Login with **GitLab** credentials as: **root**/**passw0rd**.
4. Click **New project** to create a new GitLab project.

  ![](images/gitlab-new-repo.png)
5. Enter the new project name as: **cloudnative_sample_app_deploy**, set **Visibility Level** as **Public** and click **Create project**.
...
9. Run the git commands to push the application code to the **GitLab** repo. 
  ...
  ```
10. When it is done, refresh your **GitLab** page, you see something like this:
...
5. If you see the **Warning: Potential Security Risk Ahead** message, click **Advanced**>**Accept the Risk and continue**.
 
  The ArgoCD page is displayed.

  ![](images/argocd-ui-page.png)

6. Login with the username and password with the information you obtained above and click **SIGN IN**.

  ![](images/argocd-ui-login.png)
7. From the ArgoCD Home page, click **Settings**, then click **Projects**.
...
9. Enter the project Name and Description as: **dv0100st** and click **Create**.
...
10. Click the project link to access its details.

  ![](images/argocd-project-link.png)

11. First you need to add information to the **RESOURCE REPOSITOIES**, click **EDIT** next to it.

  ![](images/argocd-edit-resource-repos.png)
  
12. In the **RESOURCE REPOSITOIES** page, click **ADD SOURCE** link.

  ![](images/argocd-add-resource.png)

13. After you clicked the **ADD SOURCE** link, the default source setting (*****) is added to the source list, accept it and click **SAVE** to save the change.

  ![](images/argocd-resource-added.png)
14. Go to **DESTINATIONS** section, click **EDIT**, then click **ADD DESTINATION** to add **Server** and **Namespace** to **DESTINATIONS** (using the default ** * ** settings).

15. Go to **CLUSTER RESOURCE ALLOW LIST** section, click **EDIT**, then click **ADD RESOURCE** to add **Kind** and **Group** to **CLUSTER RESOURCE ALLOW LIST** (using the default ** * ** settings). When you are done, your project settings should look like this:
...
2. Add the following details and click **Create** to finish.  
...
4. Next you need to push the change to the **GitLab** repo with commands:
...
5. Go to your ArgoCD **simple** application and click **SYNC** > **SYNCHRONIZE**.
  ...
  The ArgoCD will sync the application with your **GitLab** repo to get the latest changes and update the deployment.
...
2. Change the replica value from **1** to **4**, and save the change.

  ![](images/argocd-deployment-yaml-save.png)  

3. Push the change to the Git repo with commands:
...
2. Scroll down to the **Sync Policy** section and click **Enable Auto-Sync**.

  ![](images/argocd-sync-policy-auto-sync.png)

3. Click **OK** to enable ArgoCD **Auto-Sync** policy.
...
4. Go back to OCP web console to view the deployment, you see now the deployment has **4** replicas.

  ![](images/argocd-4-pods-running.png)

4. Click to change the replica to **2**.
...
2. Scroll down to the **Sync Policy** section and click **Enable Self-Heal**

  ![](images/argocd-enable-self-heal.png)

3. Click **OK** to enable it, you can see that ArgoCD is enforcing the deployment settings defined in the Git repo and brings back all **4** pods.

  ![](images/argocd-4-pods-set.png)

4. Go back to OCP web console to view the deployment, you see now the deployment is set back to **4** replicas as designed.

5. Try to make any changes the replica again, you can see that system will automatically adjust it back to **4** which is defined in the **deployment.yaml** file.
