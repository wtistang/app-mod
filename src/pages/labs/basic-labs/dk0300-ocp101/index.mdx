...
* get familiar with the OCP web console environment.
* learn how to use OCP cluster resources and tools to deploy and manage the cloud applications inside the OCP web console 
* learn how to work with OCP cluster from command line
...
5. Switch to a specific project **dk0300st** from the **default** project. 
...
3. List pods again with command **oc get pods** and note that a new instance of the pod has been created as expected. The deployment specified 1 instance, so the controller tries to maintain 1 instance.
  ...
  ```
  NAME                      READY   STATUS    RESTARTS   AGE
  example-5fb6876865-qkj4r   1/1     Running   0          13s
  ```

4. To increse the number of pod instances, you can patch the resource in one of two ways:
...
5. List the pods to show 2 pods are running.
