...
* work with Istio service mesh in OCP cluster
* use metrics, logging and tracing to observe services
* control traffic using A/B tests and canary deployments
...
* Enable SSL for traffic coming into your cluster (HTTPS)
...
A/B testing is a method of performing identical tests against two separate service versions in order to determine which performs better to test new features. A/B testing is NOT blue-green deployments which is about releasing new version safely to production and rolling back predictably.

To prevent Istio from performing the default routing behavior between the original and modernized service, define the following rules:
