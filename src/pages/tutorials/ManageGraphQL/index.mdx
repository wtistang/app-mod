...
In this tutorial, you use the IBM App Mod Workshop 2021.1.1 to define GraphQL APIs that proxy to a backend GraphQL server. GraphQL is a query language for APIs that gives an application client greater control over what data it retrieves in an API request when compared with a REST API request.
...
App Mod Workshop API Connect enables you to create a GraphQL API proxy definition that proxies a backend GraphQL server.  API Connect extends the GraphQL standard by providing, in a GraphQL API definition, the ability to configure a range of settings that are used to calculate the complexity of a GraphQL request and an associated cost that counts towards the rate limit.

***

## Create a GraphQL Proxy API

1.Open a browser and go to your **App Mod Workshop Platform Navigation**. On the **API Lifecycle management** box, click to open your **API Connect** Runtime.
...
To learn more about App Mod Workshop and API Life Cycle Management, visit <https://www.ibm.com/cloud/cloud-pak-for-integration>
