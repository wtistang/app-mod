...
In this tutorial, you use the IBM App Mod Workshop 2021.1.1 to deploy an App Integration flow on containers and expose it as a secure rate-limited API on RedHat OpenShift 4.6.
...
As this is a brand-new deployment of the App Mod Workshop, all instances of integrations, message queues, and event streams are deployed as microservices. We need to authorize the Message Queue service to accept incoming data from the integration running on a separate server.
App Mod Workshop provides a single solution for all of your enterprise integration needs. The platform provides a comprehensive set of industry-leading capabilities. Use any of them on their own or together through a single interface. Create, manage, and monitor all of your integrations across SaaS applications, messaging, streams, APIs, high-speed transfer, and more.
...
7.Open a browser and go to your **App Mod Workshop Platform Navigation**. On the **Message** box, click to open your **MQ Queue Manager Runtime**.
...
This task covers opening and examining an application integration flow in the IBM App Connect Enterprise Toolkit. With the Toolkit you can build powerful and complex integration applications, services, and APIs quickly and easily using a visual designer. Your integration solutions can be directly deployed to the App Mod Workshop on IBM Cloud Pak running on-premise, in any cloud, or combinations of both.
...
1.To push our new API to **Asset Repository** you will use the API Manager. Back to the App Mod Workshop Navigator Platform browser tab. Open the **Menu** and on **Run** section, select **APIs**.
...
Now that you’ve made your back-end integrations ready for external distribution, your developer community is able to access the APIs via a developer portal. The developer portal is included in the platform and provides a full-featured experience to onboard and nurture your API consumers.  For more information about App Mod Workshop, access [here](https://www.ibm.com/cloud/cloud-pak-for-integration).
