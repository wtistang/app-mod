...
## Prerequisites
  ...
  - You need to have your personal CP4I on ROKS environment. [Check here how to request it](https://wtistang.github.io/app-mod/cp4i-on-roks/)
  - You need to have the OC CLI and you should have logged in your ROKS cluster via command line. [Check here how to do it](https://wtistang.github.io/app-mod/cp4i-on-roks/#access-your-roks-cluster-by-command-line)
...
4.Open a browser and go to your **App Mod Workshop Platform Navigation**. From here, you are able to navigate to all the integration runtimes and capabilities available in the platform. Capabilities include: EventStreams, API Connect, App Connect Dashboard, App Connect Designer , Asset Repository and Operations Dashboard. Let's explore the Event streaming runtimes. On the *App Mod Workshop Home*, scroll down to see the **Event streaming** box, and click to open your **es-dev** instance.
...
5.A message was sent from App Connect (Integration Server) to IBM EventStreams. Let's check it! Back to App Mod Workshop home page, open the **Menu** again, and on **Run** section, now click **Kafka clusters**.
...
IBM App Mod Workshop Operations Dashboard has adopted OpenTracing API specification for collecting tracing data. OpenTracing is comprised of an API specification for distributed tracing, frameworks and libraries that have implemented the specification and documentation.

o	Trace: The description of a transaction as it moves through IBM App Mod Workshop platform.
...
Now that you’ve created a topic in Kafka (Event Streams), applications are able to subscribe and received data. To try out more labs, go to App Mod Workshop Demos. For more information about the App Mod Workshop, go to https://www.ibm.com/cloud/cloud-pak-for-integration.
